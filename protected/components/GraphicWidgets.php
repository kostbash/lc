...
class GraphicWidgets {
    ...
    function pick($name, $params) {
        ...
        switch ($name) {
            case "clock": $widget = new WidgetClock($params);
                break;
            case "div_col": $widget = new WidgetDivCol($params);
                break;
            case "inp": $widget = new WidgetInput($params);
                break;
            case "ex": $widget = new WidgetExpression($params);
                break;
            case "mult_col": $widget = new WidgetMultCol($params);
                break;
            case "add_col": $widget = new WidgetAddCol($params);
                break;
            case "sub_col": $widget = new WidgetSubCol($params);
                break;
            case "select": $widget = new WidgetSelect($params);
                break;
            case "radio": $widget = new WidgetRadio($params);
                break;
        }
    ...
    /*
     * Составляем массив виджетов, по этому названию создаются привязки
     * к классам виджетов в методе pick(), а также вьюшки, которые
     * строят форму добавления виджета
     */
