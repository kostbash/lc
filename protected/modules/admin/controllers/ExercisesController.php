...
class ExercisesController extends Controller {
    ...
    public function accessRules() {
        return array(
            array('allow',
                'actions' => array('create', 'update', 'updatebyajax', 'delete', 'index', 'savechange', 'skillsbyajax', 'skillsnotidsajax', 'skillsbyidsajax', 'createfromgroup', 'SWFUpload', 'massdelete', 'gethtmlvisual', 'gethtmlvariant', 'saveParams', 'buildbuttonswindow', 'buildexpressionbuttons'),
                'roles' => array('editor'),
            ),
    ...
    public function actionUpdate($id) {
        ...
        if (isset($_POST['Exercises'])) {
            ...
            if ($model->save()) {
                if ($_POST['Skills']['ids']) {
                    foreach ($model->ExerciseAndSkills as $eSkill)
                        $eSkill->delete();
                    $exerciseSkills = new ExerciseAndSkills();
                    foreach ($_POST['Skills']['ids'] as $id_skill) {
                        $exerciseSkills->id_exercise = $model->id;
                ...
                if ($model->id_visual == 9) {
                    ...
                    if ($question->save()) {
                        ...
                        if ($_POST['Exercises']['answers']) {
                            foreach ($model->Answers as $eAnswer)
                                $eAnswer->delete();
                            foreach ($_POST['Exercises']['answers'] as $answerAttr) {
                                // проверяем что пробел действительно существует
                                if (in_array($answerAttr['number_space'], $numberSpaces)) {
                ...
                if ($_POST['Exercises']['Comparisons']) {
                    foreach ($model->Comparisons as $eComparison)
                        $eComparison->delete();
                    foreach ($_POST['Exercises']['Comparisons'] as $comparison) {
                        $comparisonAttr = array();
                        foreach ($comparison as $nameAttr => $answerAttr) {
    ...
    public function actionBuildButtonsWindow($widget) {
        ...
        $objWidget = $gw->pick($widget, '');

        $this->renderPartial('/exercises/modals/_addmodal', array('widget' => $widget, 'objWidget' => $objWidget), false, true);
        Yii::app()->end();
    }
