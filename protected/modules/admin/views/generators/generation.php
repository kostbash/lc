<script type='text/javascript'>
  ...
  if ( event.which == 13 ) {
    event.preventDefault();
        ...
        $('#add-second-skills .dropdown-menu li').live('click', function(){
            ...
            if(id)
            {
                ...
                skillExist = skillsContainer.find('.second-skill[data-id='+id+']');
                if(!skillExist.length)
                    skillsContainer.append(getSecondSkills(id, name));
            }
            current.closest('.input-group-btn').removeClass('open');
            return false;
        });
            ...
            if(checked.length && skills.length)
            {
                checked.each(function(n, check)
                {
                    ...
                    skills.each(function(k, skill)
                    {
                        ...
                        skillExist = skillsContainer.find('.first-skill[data-id='+skill_id+']');
                        firstExist = skillsContainer.find('.first-skill[data-type="first"]');
                        if(!skillExist.length && !firstExist.length)
        ...
        $('#insert-second-skills').click(function(){
            checked = $('.select-on-check:checked');
            skills = $('#add-second-skills .skills .skill');
            if(checked.length && skills.length)
            {
                checked.each(function(n, check)
                {
                    ...
                    skills.each(function(k, skill)
                    {
                        ...
                        skill_id = skill.data('id');
                        skill_name = skill.find('.name').html();
                        skillExist = skillsContainer.find('.second-skill[data-id='+skill_id+']');
                        if(!skillExist.length)
                            skillsContainer.append(getSecondSkills(skill_id, skill_name, check.val()));
                    });
                });
            }
            return false;
    ...
    {
        result = '<div class="skill first-skill clearfix" data-id='+id+' data-type="first">';
    ...
    function getSecondSkills(id, name, number)
    {
        result = '<div style="background:#75f7f7" class="skill second-skill clearfix" data-id='+id+'>';
        result += '<p class="name" style="">'+name+'</p>';
        result += '<p class="remove">&times;</p>';
        if(number)
            result += '<input type="hidden" value="'+id+'" name="Exercises['+number+'][SecondSkillsIds][]" />';
        result += '</div>';
        return result;
    }
</script>
        ...
        <div class="section">
            <div class="row">
                <div class="col-lg-2 col-md2">
                    Для выделенных задать основное умение
                </div>
            ...
            <div class="row">
                <div class="col-lg-2 col-md2">
                    Для выделенных задать второстепенные умения
                </div>
                <div class="col-lg-4 col-md-4">
                    <div id="add-second-skills">
                        <div class="input-group mydrop ">
                            <?php echo CHtml::textField("SecondSkills[name]", '', array('placeholder'=>'Введите название умения', 'class'=>'form-control input-sm', 'id'=>'searchSkill', 'autocomplete'=>'off')); ?>
                            <div class="input-group-btn">
                                <button type="button" class="btn btn-info btn-sm dropdown-toggle" data-toggle="dropdown" tabindex="-1">
                                    <span class="caret"></span>
                                </button>
                ...
                <div class="col-lg-2 col-md2">
                    <a id="insert-second-skills" href="#" class="btn btn-success btn-icon btn-sm"><i class="glyphicon glyphicon-plus"></i>Задать умения</a>
                </div>
            </div>
